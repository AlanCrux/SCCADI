...
/**
 * Controller de la ventana IUAdministrarAlumnos, permite consultar, editar o elimina a un alumno
...
public class IUAdministrarAlumnosController implements Initializable {
  ...
  /**
   * Este metodo carga la informacion del alumno seleccionado en los cuadros de texto con su
   * respectiva imagen
   */
  ...
  /**
   * Este metodo convierte la cadena binarios que arroja la base de datos en una imagen
   *
   * @param blobImage parametro de tipo Blolb que representa una imagen en mysql
  ...
  public void eliminarAlumno() {
    ...
    for (int i = 0; i < listaAlumno.size(); i++) {
      if (listaAlumno.get(i).getMatricula().equals(seleccionado.getMatricula())) {
        ...
        try {
          eliminarAlumno = alumno.eliminarAlumno(seleccionado.getMatricula());
        } catch (Exception ex) {
          Logger.getLogger(IUAdministrarAlumnosController.class.getName()).log(Level.SEVERE, null, ex);
        }
        if (eliminarAlumno) {
  ...
  /**
   * Este metodo nos permite editar la informacion de un alumno registrado en la base de datos
   */
  @FXML
  public void editarAlumno() {
    try {
      ...
      } else if (alumno.actualizarAlumno(matricula, obtenerDatos())) {
        deshabilitarCuadros();
  ...
  /**
   * Este metodo es un auxiliar, habilita los cuadros de texto cuando es necesario
   */
  ...
  /**
   * Este metodo obtiene la informacion proporcionada en la pantalla y crea un objeto de tipo Alumno
   *
  ...
  /**
   * Este metodo es un auxiliar, deshabilita los cuadros de texto cuando es necesario
   */
