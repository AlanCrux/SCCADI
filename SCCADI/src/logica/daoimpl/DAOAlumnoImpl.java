...
public class DAOAlumnoImpl extends Conexion implements DaoAlumno {
    ...
    /**
     * Obtiene un Alumno. Este método busca todos los Alumnos registrados en la
     * BD y los regresa en una lista
     *
    ...
    /**
     * Obtiene un Alumno. Este método utiliza la clave matricula para obtener un
     * Alumno en especial
     *
   ...
   */
    ...
    public List<Alumno> obtenerAlumnosPorSeccion(int noPersonal) throws Exception {
        List<Alumno> alumnos = null;
        try {
            this.connection();
            PreparedStatement st = this.conn.prepareStatement("select al.nombre, al.fotografia, al.matricula "
                    + "from alumno al join seccion join inscripcion join asesor where asesor.noPersonal =? "
                    + "and inscripcion.matricula = al.matricula and seccion.nrc = inscripcion.nrc and "
                    + "asesor.noPersonal = seccion.noPersonal;");
